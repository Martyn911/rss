<template>
    <inertia-head>
        <title v-if="tag">Posts by {{ tag }} tag</title>
        <title v-if="feed">Posts by {{ feed.name }} feed</title>
        <title v-if="!feed && !tag">RSS aggregator</title>
    </inertia-head>
    <div class="md:flex h-full min-h-0 max-w-7xl mx-auto">
        <div
            class="md:w-1/3 xl:w-1/4 md:flex md:justify-end md:border-r dark:border-black px-6 bg-gray-50 dark:bg-gray-800 md:px-12 md:overflow-auto py-3">
            <div class="md:hidden mb-2">
                <button @click="mobileShowOptions = !mobileShowOptions"
                    ...
                    <svg :class="{'rotate-90': mobileShowOptions}" xmlns="http://www.w3.org/2000/svg" width="16"
                ...
                <div class="mb-8">
                    ...
                    <form @submit.prevent="submitSearch">
                        <input type="text"
                               ...
                               @change="submitSearch"
                               class="border border-gray-300 dark:border-black rounded px-2 py-1 w-full bg-white dark:bg-gray-900 dark:text-gray-100">
        ...
        <div class="md:w-2/3 xl:w-3/4 px-6 md:px-12 md:overflow-auto md:h-full py-2">
            <div
                :class="{'max-w-lg': postFormat === 'card', 'max-w-2xl': postFormat === 'list', 'w-full': postFormat === 'compact'}">

                <div class="flex items-center justify-between mb-2">
                    <h2 class="font-bold text-black dark:text-gray-400">
                        <Link href="./">Posts</Link>
                        <span v-if="tag"> / #{{ tag }}</span>
                        <span v-if="feed && feeds.length === 1"> / <span>{{ feeds[0].name }}</span></span>
                        <span v-if="search"> by search query: {{ search }}</span>
                    </h2>
                ...
                <div class="py-2 flex">
                    <Link v-if="page > 1" :href="pagePath" :data="{page: page-1}"
                          class="cursor-pointer text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 font-bold py-2 hover:text-black">
                        &laquo; Previous Page
                    </Link>
                    <Link v-if="posts.length === 100" :href="pagePath" :data="{page: page + 1}"
                          class="ml-auto cursor-pointer text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 font-bold py-2 hover:text-black">
                        Next Page &raquo;
                    </Link>
                </div>
