<template>
    <div
        ...
        <div class="flex text-sm text-gray-600 dark:text-gray-400 items-center flex-wrap">
            <Link :href="`f/${post.feed.id }`"
                  :style="{color: 'rgb(129 140 248)'}"
                  class="font-bold text-xs block">{{ post.feed.name }}
            </Link>
        ...
        <div :class="{'flex items-center gap-5': formatList && post.thumbnail}">
            ...
            <div :class="{'whitespace-nowrap overflow-ellipsis overflow-hidden': formatCompact, 'flex-1': formatList}">
                <h3 class="font-bold text-black dark:text-gray-300"
                    :class="{'text-lg': formatCard, 'text-md': formatList, 'inline': formatCompact}">
                    <a :href="post.url" target="_blank" class="hover:underline" rel="nofollow">{{ post.title }}</a>
                </h3>
...
export default {
    ...
    computed: {
        ...
        relatedPublishedTime() {
            return timestampToRelativeTime(Date.parse(this.post.published_at) / 1000);
        },
        formatCard() {
