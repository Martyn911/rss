<template>
    <div class="py-1 my-2">
        <h4 class="font-bold text-black dark:text-gray-400" :style="{color: 'rgb(129 140 248)'}">
            <Link :href="`f/${feed.id}`">{{ feed.name }}</Link>
        </h4>
        <div
            class="font-mono text-gray-600 dark:text-gray-500 text-xs my-1 overflow-ellipsis whitespace-nowrap w-full overflow-hidden">
            {{ feed.url }}
        </div>
...
export default {
    ...
    methods: {
        ...
        async pollFeedStatus() {
            const resp = await axios.get(`feed/` + this.feed.id);
            if (resp?.data?.outdated) {
                this.feed.reloading = false;
                this.pendingRefresh = true;
            } else {
                this.reloadingPoll = setTimeout(this.pollFeedStatus.bind(this), 30000);
            }
        }
    },
    mounted() {
